...
void fat_fuse_log_activity(char *operation_type, fat_file target_file)
{
    ...
    fat_tree_node node = fat_tree_node_search(vol->file_tree, LOG_FILE);
    fat_file file = fat_tree_get_file(node);
    fat_file parent = fat_tree_get_parent(node);
    fat_file_pwrite(file, buf, LOG_MESSAGE_SIZE, file->dentry->file_size, parent);
}

/* Initialize fs.log file */
void log_init(){
    fat_volume vol = get_fat_volume();
    fat_tree_node log_node = fat_tree_node_search(vol->file_tree, LOG_FILE);
    if (log_node != NULL){
        printf("YA EXISTE FS.LOG\n");
        fat_file log_file = fat_tree_get_file(log_node);
        assert(log_file != NULL);
        fat_file_print_dentry(log_file->dentry);
    }
    else{
        printf("CREANDO FS.LOG\n");
        int mknod_exit = fat_fuse_mknod(LOG_FILE, 0, 0);
        assert(mknod_exit == 0);
        log_node = fat_tree_node_search(vol->file_tree, LOG_FILE);
        assert(log_node != NULL);
        fat_file log_file = fat_tree_get_file(log_node);
        assert(mknod_exit == 0);
        fat_file log_parent = fat_tree_get_parent(log_node);
        assert(log_parent != NULL);
...
int fat_fuse_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
    ...
    child = children;
    while (*child != NULL) {
        // Makes fs.log invisible to ls
        if(fat_file_cmp_path(*child, LOG_FILE) == 0){
            child++;
            continue;
        }
