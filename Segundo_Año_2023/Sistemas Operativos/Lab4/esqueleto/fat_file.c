...
static void read_cluster_dir_entries(u8 *buffer, fat_dir_entry end_ptr,
    ...
    for (disk_dentry_ptr = (fat_dir_entry)buffer; disk_dentry_ptr <= end_ptr;
        ...
        bool is_log = 
                        strncmp((char *)disk_dentry_ptr->base_name, log_name, 8) == 0 && 
                        strncmp((char *)disk_dentry_ptr->extension, LOG_FILE_EXTENSION, 3) == 0;

        if (is_log) {
            char original_name[] = LOG_FILE_BASENAME;
            disk_dentry_ptr->base_name[0] = original_name[0];
        }
...
void fat_file_delete(fat_file file, fat_file parent) {
    ...
    file->dentry->base_name[0] = FAT_FILENAME_DELETED_CHAR; // 0xe5
    write_dir_entry(parent, file->dentry, file->pos_in_parent);
